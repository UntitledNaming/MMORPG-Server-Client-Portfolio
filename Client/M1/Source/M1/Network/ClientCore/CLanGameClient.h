...
public:
	...
	// CLanClient �������̽� �Լ�
	////////////////////////////////////////////////////
	virtual void OnEnterJoinServer() = 0;                                              // Callback when completed joining server    
	virtual void OnLeaveServer() = 0;                                                  // Callback when deleted client session 
	virtual void OnRecv(CMessage* pMessage) = 0;                                       // Callback when completed recving message
	virtual void OnSend(int sendsize) = 0;                                             // Callback when completed sending message
	...
	bool  Disconnect();
	bool  SendPacket(CMessage* pMessage, ERouteType routeType, uint16 serviceID = ServiceID::NONE_SERVICE);
	bool  ReConnect();                                                                
	bool  ConnectAlive();
...
private:
	...
	bool  CreateAndSetSocket(SOCKET& outParam);                                        
	bool  ConnectTry(SOCKET inputParam);                                               
	bool  RecvPost();              
	bool  SendPost();              
